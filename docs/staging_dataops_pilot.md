...
follow-up now archives Gamma detail, CLOB orderbook, and CLOB price-history payloads:
...
The final validated run completed as `COMPLETED` with 5 archived payloads: one market
detail, two orderbooks, and two price-history payloads. The price-history requests used
token-level `market` parameters with `interval=1d` and `fidelity=60`, creating 52
additional price snapshots for the selected market. If a selected Polymarket market has no
persisted Gamma ID or token mappings, the collection run should remain `PARTIAL` with a
safe missing-identifier error. Do not fabricate token IDs, orderbook data, or price
history, and do not schedule public-read collection.
